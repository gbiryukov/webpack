	...
	apply(mainTemplate) {
		...
		mainTemplate.plugin("jsonp-script", function(_, chunk, hash) {
			...
			return this.asString([
				...
				`var timeout = setTimeout(getCompletionHandler('timeout'), ${chunkLoadTimeout});`,
				"script.onerror = script.onload = getCompletionHandler();",
				"function getCompletionHandler(reason) {",
				this.indent([
					"return function onScriptComplete() {",
					this.indent([
						...
						this.indent([
							"if(chunk) {",
							this.indent([
								"var details = reason ? ' (' + reason + ' occured)': '';",
								"chunk[1](new Error('Loading chunk ' + chunkId + ' failed.' + details));",
							]),
							"}",
